...
const notifications = computed(() => {
    const value = (company.value?.brand_settings?.notifications ?? {}) as Record<string, boolean>;
    return { email: value.email ?? true, push: value.push ?? false, telegram_bot: value.telegram_bot ?? false };
});
