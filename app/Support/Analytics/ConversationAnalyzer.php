    ...
    public function analyze(Conversation $conversation): ?ConversationAnalysis
    {
        ...
        return ConversationAnalysis::query()->updateOrCreate(
            ...
            [
                ...
                'completeness_score' => $this->clampScore($data['completeness_score'] ?? null, null),
                'clarity_score' => $this->clampScore($data['clarity_score'] ?? null, null),
                'politeness_score' => $this->clampScore($data['politeness_score'] ?? null, null),
                'redundant_messages_count' => is_numeric($data['redundant_messages_count'] ?? null) ? max(0, (int) $data['redundant_messages_count']) : null,
                'had_to_reexplain' => array_key_exists('had_to_reexplain', $data) ? (bool) $data['had_to_reexplain'] : null,
...

{
  ...
  "completeness_score": целое число 0-100 — насколько полным был ответ (учтены ли все части вопроса клиента),
  "clarity_score": целое число 0-100 — насколько понятным и чётким был ответ,
  "politeness_score": целое число 0-100 — вежливость AI/оператора,
  "redundant_messages_count": целое число — сколько сообщений в диалоге были лишними/повторяющими предыдущий ответ без новой информации,
  "had_to_reexplain": true/false — пришлось ли клиенту повторно объяснять свой вопрос, потому что его не поняли с первого раза,
