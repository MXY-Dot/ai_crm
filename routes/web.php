...
Route::middleware(['auth', EnsureTenantActive::class, EnsurePageAccess::class])->group(function () use ($dashboardPage): void {
    ...
    Route::get('/hotel-settings', fn (Request $request, DashboardData $dashboard) =>
        $dashboardPage($request, $dashboard, 'HotelSettingsPage'))->name('hotel-settings');
    Route::get('/auto-service-settings', fn (Request $request, DashboardData $dashboard) =>
        $dashboardPage($request, $dashboard, 'AutoServiceSettingsPage'))->name('auto-service-settings');
    Route::get('/education-settings', fn (Request $request, DashboardData $dashboard) =>
        $dashboardPage($request, $dashboard, 'EducationSettingsPage'))->name('education-settings');
    Route::get('/travel-settings', fn (Request $request, DashboardData $dashboard) =>
